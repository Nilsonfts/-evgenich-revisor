...
def send_end_of_shift_report_for_chat(bot, chat_id: int):
    ...
    try:
        shift_goal = shift_data.shift_goal
        analytical_conclusion = generate_analytical_summary(main_user_data, shift_goal, chat_id)
        
        append_shift_to_google_sheet(bot, chat_id, shift_data, analytical_conclusion)
        
        report_lines = generate_detailed_report(chat_id, shift_data)
        final_report_text = "\n".join(report_lines)
...
def check_user_activity(bot):
    ...
    for chat_id, shift_data in list(chat_data.items()):
        if not shift_data.main_id:
            continue

        main_id = shift_data.main_id
        user_data = shift_data.users.get(main_id)
        if not user_data:
            continue

        # Проверка тех, кто на перерыве
        if user_data.on_break:
            break_start_time_str = user_data.break_start_time
        ...
        last_voice_time_str = user_data.last_voice_time
...
def check_for_shift_end(bot):
    ...
    for chat_id_str, config in list(chat_configs.items()):
        ...
        try:
            ...
            if now_local.strftime('%H:%M') == end_time_str:
                current_shift = chat_data.get(chat_id)
                if current_shift and current_shift.main_id:
                    last_report_date_str = current_shift.last_report_date
